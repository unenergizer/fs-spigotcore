...
public class WoodCutting extends BlockBreakSkill<WoodCuttingProfileData> implements LoadsConfig {

    public WoodCutting() {
        super(YamlConfiguration.loadConfiguration(new File(FilePaths.PROFESSION_WOOD_CUTTING.toString())),
                SkillType.WOOD_CUTTING);
    }
    ...
    public void databaseSave(Player player, WoodCuttingProfileData profileData, Connection connection) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement("UPDATE fs_skill_wood_cutting SET experience=?, logs_harvested=?, logs_failed=? WHERE uuid=?");
    ...
    public ProfileData firstTimeSave(Player player, Connection connection) throws SQLException {
        ...
        PreparedStatement newPlayerStatement = connection.prepareStatement("INSERT INTO fs_skill_wood_cutting " +
    ...
    public SqlSearchData searchForData(Player player, Connection connection) {
        return new SqlSearchData("fs_skill_wood_cutting", "uuid", player.getUniqueId().toString());
    }
