...
public class Mining extends BlockBreakSkill<MiningProfileData> implements LoadsConfig {

    public Mining() {
        super(YamlConfiguration.loadConfiguration(new File(FilePaths.PROFESSION_MINING.toString())), SkillType.MINING);
    }
    ...
    public void databaseSave(Player player, MiningProfileData profileData, Connection connection) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement("UPDATE fs_skill_mining SET experience=?, ores_mined=?, ores_failed=? WHERE uuid=?");
    ...
    public ProfileData firstTimeSave(Player player, Connection connection) throws SQLException {
        ...
        PreparedStatement newPlayerStatement = connection.prepareStatement("INSERT INTO fs_skill_mining " +
    ...
    public SqlSearchData searchForData(Player player, Connection connection) {
        return new SqlSearchData("fs_skill_mining", "uuid", player.getUniqueId().toString());
    }
}
