...
public class Mining extends BlockBreakProfession<MiningProfileData> implements LoadsConfig {
    ...
    public Mining() {
        super(YamlConfiguration.loadConfiguration(new File(FilePaths.PROFESSION_MINING.toString())),
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        if (isProfileDataLoaded(player)) {
            ...
            player.sendMessage(Text.color("&aSaving your profession data..."));
            return;
        }
    }
    ...
    public ProfileData databaseLoad(Player player, Connection connection, ResultSet resultSet) throws SQLException {
        MiningProfileData miningProfileData = new MiningProfileData();

        miningProfileData.setMiningExp(resultSet.getLong("experience"));
        miningProfileData.setOresMined(resultSet.getInt("ores_mined"));
        miningProfileData.setOresFailed(resultSet.getInt("ores_failed"));
        miningProfileData.setDataLoaded(true);

        return miningProfileData;
    }
    ...
    public ProfileData firstTimeSave(Player player, Connection connection) throws SQLException {
        ...
        MiningProfileData realmProfileData = new MiningProfileData();
        realmProfileData.setMiningExp(experience);
        realmProfileData.setOresMined(oresMined);
        realmProfileData.setOresFailed(oresFailed);
